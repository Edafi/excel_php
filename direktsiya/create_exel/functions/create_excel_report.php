...
//function create_excel($group_name){
	function create_excel($group_id){
	  ...
	  $profile		= $connect->query('SELECT * FROM Practices.profiles WHERE id = '.$stream['profile_id'].' ;')->Fetch();
	  $faculty		= $connect->query('SELECT * FROM Practices.faculty WHERE id = '.$profile['faculty_id'].';')->Fetch();
	  $group_name   = $stream['name'].'-'.$group['group_number'];
	  $group_parts 		= explode("-", $group_name);

      $stream_name 		= $steam['name']."-".$group['number'];
      $stream_id 		= $stream["id"];
      $profile_id  	    = $stream["profile_id"];
      $stream_code      = $stream["code"];
      $full_stream_name = $stream["full_name"];
	  ...
	  ob_end_clean(); 
		//$path = '../../uploads/'.$group_name.'.xls';
	  $path = '/var/www/html/sotrudniku/praktika/direktsiya/uploads/'.$group_name.'.xls';
	  ...
	  $resultset = $connect->query('SELECT count(*) FROM Practices.templates WHERE group_id = '.$group['id'].';')->Fetch()["count(*)"];
	  if (!$resultset){
		  $connect->query('INSERT INTO Practices.templates (group_id, name ,decanat_check) VALUES ("'.$group['id'].'","'.$path.'",0);');
	  }
